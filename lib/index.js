...
var json = require('xmaxjs-json');
var Fcbuffer = require('xmaxjs-fcbuffer');
var api = require('xmaxjs-api');
...
function development(Network) {
  return function () {
    ...
    var xmaxConfig = Object.assign({}, {
      transactionLog: consoleObjCallbackLog(config.verbose) }, config);
    return createXMax(xmaxConfig, Network, network);
  };
}

XMax.Testnet = development(api.Testnet);
XMax.Localnet = development(api.Localnet);
// XMax.Mainnet = config => ..

function createXMax(config, Network, network) {
  ...
  var xmax = mergeWriteFunctions(config, Network);

  if (!config.signProvider) {
    config.signProvider = defaultSignProvider(xmax, config);
  }
...
function mergeWriteFunctions(config, Network) {
  ...
  throwOnDuplicate(merge, network, 'Conflicting methods in XMax and Network Api');
  Object.assign(merge, network);

  var writeApi = writeApiGen(Network, network, structs, config);
  throwOnDuplicate(merge, writeApi, 'Conflicting methods in XMax and Transaction Api');
...
*/
var defaultSignProvider = function defaultSignProvider(xmax, config) {
  return function (_ref) {
    ...
    return xmax.getRequiredKeys(transaction, pubkeys).then(function (_ref2) {
