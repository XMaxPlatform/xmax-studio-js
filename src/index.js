...
function development(Network) {
  return (config = {}) => {
    ...
    const xmaxConfig = Object.assign({}, {
...
function createXMax(config, Network, network) {
  ...
  const xmax = mergeWriteFunctions(config, Network)
...
*/
const defaultSignProvider = (xmax, config) => ({sign, buf, transaction}) => {
  ...
  return xmax.getRequiredKeys(transaction, pubkeys).then(({required_keys}) => {
